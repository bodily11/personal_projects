...
function scene:create( event )
    ...
    userStats = event.params["userStats"]
    levelChosen = event.params["levelChosen"]
    if userStats.likes then
        user_stats = userStats
    else
        user_stats = userStats[levelChosen]
    end
    
    levelID = user_stats.levelID
    levelType = user_stats.levelID
    levelDifficulty = user_stats.levelDiff
    ...
    function backNextScene( event )
        composer.removeScene("specificuserstats")
        composer.gotoScene("alluserstats", { effect = "crossFade", time = 500, params = {}} )
    end
