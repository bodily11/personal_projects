    ...
    elseif ( phase == "did" ) then
        ...
        local function updateTime()
            ...
            if secondsLeft == 0 then
                ...
                composer.removeScene( "associations" )
                composer.gotoScene("pick_best_three_between", { effect = "crossFade", time = 500, params = {finalLevelNumber = finalLevelNumber, levelType = levelType, levelDiff = levelDifficulty}} )
        ...
        --local tile_clicked_on = event.params["levelName"]
        --local number = string.match(tile_clicked_on, "%d+")
        --local number_index = string.find(tile_clicked_on,number)
        ...
        file = nil
        print(finalLevelNumber)
        
        level_prompt = fileLines[finalLevelNumber]
        print(level_prompt)
        local number_index = string.find(level_prompt,":")
        local levelWord = string.sub(level_prompt, 1, number_index - 1)
        local levelProblem = string.sub(level_prompt, number_index + 2)
        
        textOptions = {
            parent = sceneGroup,
            y = mui.getScaleVal(160),
            x = display.contentWidth / 2,
            name = "title-main",
            text = "1: "..levelWord,
            align = "center",
        ...
        local problem = mui.newText(textOptions2)
