...
function scene:create( event )
    ...
    local function loginUser( event )
        if ( event.phase == "onTarget" ) then
            ...
            local username = mui.getWidgetProperty("username","value")
            ...
            local body = "email=" .. email:urlEncode() .. "&password=" .. password:urlEncode() .. "&username=" .. username:urlEncode()
            
            local params = {}
            params.headers = headers
            params.body = body
            
            network.request( "https://yourcreativitygym.com/submitlogin", "POST", networkListener23, params )
    ...
    function goToLogin2 (event)
        composer.gotoScene("login2", { effect = "crossFade", time = 500 } )
    end
    mui.newRoundedRectButton({
        parent = sceneGroup,
        name = "login",
