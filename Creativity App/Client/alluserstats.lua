...
function scene:create( event )
    ...
    function networkListener( event )
        ...
        elseif user_stats.likes then
            ...
            table.insert(user_scores,{x.levelType,x.levelDiff,x.levelID,user_score})
        ...
        if user_stats == nil then
            ...
            for i,x in ipairs(user_scores) do
                table.insert(list_options,{ key = "key", text = "text", value = "value", isCategory = false, columns = {
                            { text = "  "..x[1].."\n  "..x[2].." #"..x[3], value = "5A", align = "left" },
                            { text = x[4], value = "5A", align = "center" },
                        }})
            end
        end
        
        function testontouch( event )
            --minus one because the table category on top counts as one
            local muiTargetIndex = mui.getEventParameter(event, "muiTargetIndex") - 1
            
            --composer go to new place
            composer.removeScene("alluserstats")
            composer.gotoScene("specificuserstats", { effect = "crossFade", time = 500, params = {userStats = user_stats,levelChosen = muiTargetIndex}} )
    ...
    function backNextScene( event )
        composer.removeScene("alluserstats")
        composer.gotoScene("menu", { effect = "crossFade", time = 500, params = {}} )
    end
    ...
    mui.newRoundedRectButton({
        ...
        textColor = { 0, .6, 1, 1},
        touchpoint = true,
        callBack = backNextScene,
    })
            
    end
    
    network.request( "https://www.yourcreativitygym.com/getalluserstats?userID="..userSession.username, "GET", networkListener )
